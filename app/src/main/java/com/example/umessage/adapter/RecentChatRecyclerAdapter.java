package com.example.umessage.adapter;
...
import com.example.umessage.ChatActivity;
    ...
    protected void onBindViewHolder(@NonNull ChatroomModelViewHolder holder, int position, @NonNull ChatroomModel model) {
        FireBaseUtil.getOtherUserFromChatroom(model.getUserIds()).get().addOnCompleteListener(task -> {
            if(task.isSuccessful())
            {
                ...
                FireBaseUtil.getOtherProfilePicStorageReference(otherUserModel.getUserId()).getDownloadUrl().addOnCompleteListener(t->{
                    if(t.isSuccessful())
                    {
                        Uri uri = t.getResult();
